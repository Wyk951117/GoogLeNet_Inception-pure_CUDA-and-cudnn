...
/**
 * concatenate layers in the direction of channels
 * 
 * @param input1          first input to concat, same for 2/3/4
 * @param num1            the number of elements in input1, same for 2/3/4
 * @param maxNum          the number of elements in output
 * @param output          output matrix
 * @param numPerBatch     number of elements per batch
 */
__global__ void concat(float* input1, float* input2, float* input3, float* input4,
                        size_t num1, size_t num2, size_t num3, size_t num4, size_t maxNum,
                        float* output, const int numPerBatch)
{
...
/**
 * reverse of concatenation for backward pass
 * 
 * @param output1          first output, same for 2/3/4
 * @param num1            the number of elements in output1, same for 2/3/4
 * @param maxNum          the number of elements in input
 * @param output          input matrix
 * @param numPerBatch     number of elements per batch
 */
__global__ void decat(float* output1, float* output2, float* output3, float* output4,
    size_t num1, size_t num2, size_t num3, size_t num4, size_t maxNum,
    float* input, const int numPerBatch)
{
    size_t i = blockDim.x * blockIdx.x + threadIdx.x;
...
struct TrainingContext
{
    ...
    cudnnTensorDescriptor_t dataTensor, conv1Tensor, conv1BiasTensor, conv2Tensor, conv2BiasTensor,
                            conv3Tensor, conv3BiasTensor, conv4Tensor, conv4BiasTensor,
                            pool1Tensor, fc1Tensor, fc2Tensor, concat1Tensor;
    cudnnFilterDescriptor_t conv1filterDesc, conv2filterDesc, conv3filterDesc, conv4filterDesc;
    cudnnConvolutionDescriptor_t conv1Desc, conv2Desc, conv3Desc, conv4Desc;
    cudnnConvolutionFwdAlgo_t conv1algo, conv2algo, conv3algo, conv4algo;
    cudnnConvolutionBwdFilterAlgo_t conv1bwfalgo, conv2bwfalgo, conv3bwfalgo, conv4bwfalgo;
    cudnnConvolutionBwdDataAlgo_t conv2bwdalgo, conv3bwdalgo, conv4bwdalgo;  // no need for data algo for conv1
    ...
    FullyConnectedLayer& ref_fc1, ref_fc2;  // what is this for?
    ...
    TrainingContext(int gpuid, int batch_size,
                    ...
                    MaxPoolLayer& pool1, FullyConnectedLayer& fc1, FullyConnectedLayer& fc2) : ref_fc1(fc1), ref_fc2(fc2), m_gpuid(gpuid)
    {
        ...
        checkCUDNN(cudnnCreateTensorDescriptor(&conv4Tensor));
        checkCUDNN(cudnnCreateTensorDescriptor(&conv4BiasTensor));
        ...
        checkCUDNN(cudnnCreateTensorDescriptor(&concat1Tensor));
        ...
        checkCUDNN(cudnnCreateFilterDescriptor(&conv4filterDesc));
        ...
        checkCUDNN(cudnnCreateConvolutionDescriptor(&conv4Desc));

        checkCUDNN(cudnnCreatePoolingDescriptor(&poolDesc));            

        
        // Set tensor descriptor sizes (need to compute the sizes)
        ...
        checkCUDNN(cudnnSetTensor4dDescriptor(conv3BiasTensor,
                                            ...
                                            1, 1));
        checkCUDNN(cudnnSetTensor4dDescriptor(conv4BiasTensor,
                                            CUDNN_TENSOR_NCHW,
                                            CUDNN_DATA_FLOAT,
                                            1, conv4.out_channels,
                                            1, 1));
        checkCUDNN(cudnnSetTensor4dDescriptor(concat1Tensor,
                                            CUDNN_TENSOR_NCHW,
                                            CUDNN_DATA_FLOAT,
        ...
        // TODO: compute the size of differnet tensors later
        ...
        workspace = std::max(workspace, SetFwdConvolutionTensors(conv2, conv1Tensor, conv2Tensor, conv2filterDesc, conv2Desc, conv2algo));
        workspace = std::max(workspace, SetBwdConvolutionTensors(conv1Tensor, conv2Tensor, conv2filterDesc, conv2Desc, &conv2bwfalgo, &conv2bwdalgo));

        workspace = std::max(workspace, SetFwdConvolutionTensors(conv3, conv1Tensor, conv3Tensor, conv3filterDesc, conv3Desc, conv3algo));
        workspace = std::max(workspace, SetBwdConvolutionTensors(conv1Tensor, conv3Tensor, conv3filterDesc, conv3Desc, &conv3bwfalgo, &conv3bwdalgo));

        workspace = std::max(workspace, SetFwdConvolutionTensors(conv4, conv1Tensor, conv4Tensor, conv4filterDesc, conv4Desc, conv4algo));
        workspace = std::max(workspace, SetBwdConvolutionTensors(conv1Tensor, conv4Tensor, conv4filterDesc, conv4Desc, &conv4bwfalgo, &conv4bwdalgo));
    ...
    ~TrainingContext()
    {
        ...
        checkCUDNN(cudnnDestroyTensorDescriptor(conv4Tensor));
        checkCUDNN(cudnnDestroyTensorDescriptor(conv4BiasTensor));
        ...
        checkCUDNN(cudnnDestroyFilterDescriptor(conv4filterDesc));
        ...
        checkCUDNN(cudnnDestroyConvolutionDescriptor(conv4Desc));
                                    ...
                                    cudnnConvolutionFwdAlgo_t& algo)
    {
        ...
        int hp = (conv.padding) ? (conv.kernel_size - 1) / 2 : 0;
        int wp = (conv.padding) ? (conv.kernel_size - 1) / 2 : 0;
        ...
        // Find dimension of convolution output automatically given conv desc, filter desc and src desc
    ...
    {        
        float alpha = 1.0f, beta = 0.0f;   // TODO what is the meaning of alpha and beta
        checkCudaErrors(cudaSetDevice(m_gpuid));
        ...
        checkCUDNN(cudnnConvolutionForward(cudnnHandle, &alpha, dataTensor,
                                           ...
                                           conv1Tensor, conv1));  // results stored in conv1 and conv1Tensor with conv1Tensor as the descriptor
        checkCUDNN(cudnnAddTensor(cudnnHandle, &alpha, conv1BiasTensor,
                                  pconv1bias, &alpha, conv1Tensor, conv1));  // add the bias term of conv1 to conv1 and conv1Tensor
        

        // Conv2 layer
        checkCUDNN(cudnnConvolutionForward(cudnnHandle, &alpha, conv1Tensor,
                                            conv1, conv2filterDesc, pconv2, conv2Desc,
        ...
        // Conv3 layer
        checkCUDNN(cudnnConvolutionForward(cudnnHandle, &alpha, conv1Tensor,
                                            conv1, conv3filterDesc, pconv3, conv3Desc,
        ...
        // Conv4 layer
        checkCUDNN(cudnnConvolutionForward(cudnnHandle, &alpha, conv1Tensor,
                                            conv1, conv4filterDesc, pconv4, conv4Desc,
                                            conv4algo, workspace, m_workspaceSize, &beta,
                                            conv4Tensor, conv4));
        checkCUDNN(cudnnAddTensor(cudnnHandle, &alpha, conv4BiasTensor,
                                pconv4bias, &alpha, conv4Tensor, conv4));
    ...
    // void Backpropagation(ConvBiasLayer& layer_conv1, MaxPoolLayer& layer_pool1, ConvBiasLayer& layer_conv2, MaxPoolLayer& layer_pool2,  // not clear what this line is for
    ...
    void Backpropagation(ConvBiasLayer& layer_conv1, ConvBiasLayer& layer_conv2, ConvBiasLayer& layer_conv3, ConvBiasLayer& layer_conv4, MaxPoolLayer& layer_pool1,
    ...
    {    
        ...
        // Compute derivative with respect to data (for previous layer): dfc2 = pfc2*dfc2smax (500x10*10xN)
        ...
        checkCUDNN(cudnnPoolingBackward(cudnnHandle, poolDesc, &alpha, 
                                        pool1Tensor, pool1, pool1Tensor, dfc1_4,
                                        conv1Tensor, conv1, &beta, conv1Tensor, dpool1));
        
        // Conv2 layer
        checkCUDNN(cudnnConvolutionBackwardBias(cudnnHandle, &alpha, conv2Tensor,
                                                dfc1_1, &beta, conv2BiasTensor, gconv2bias));

        checkCUDNN(cudnnConvolutionBackwardFilter(cudnnHandle, &alpha, conv1Tensor,
                                                  conv1, conv2Tensor, dfc1_1, conv2Desc,
        ...
        checkCUDNN(cudnnConvolutionBackwardData(cudnnHandle, &alpha, conv2filterDesc,
                                                pconv2, conv2Tensor, dfc1_1, conv2Desc, 
                                                conv2bwdalgo, workspace, m_workspaceSize,
                                                &beta, conv1Tensor, dconv2));

        // Conv3 layer
        checkCUDNN(cudnnConvolutionBackwardBias(cudnnHandle, &alpha, conv3Tensor,
                                                dfc1_2, &beta, conv3BiasTensor, gconv3bias));

        checkCUDNN(cudnnConvolutionBackwardFilter(cudnnHandle, &alpha, conv1Tensor,
                                                conv1, conv3Tensor, dfc1_2, conv3Desc,
        ...
        checkCUDNN(cudnnConvolutionBackwardData(cudnnHandle, &alpha, conv3filterDesc,
                                                pconv3, conv3Tensor, dfc1_2, conv3Desc, 
                                                conv3bwdalgo, workspace, m_workspaceSize,
                                                &beta, conv1Tensor, dconv3));
                
        // Conv4 layer
        checkCUDNN(cudnnConvolutionBackwardBias(cudnnHandle, &alpha, conv4Tensor,
                                                dfc1_3, &beta, conv4BiasTensor, gconv4bias));

        checkCUDNN(cudnnConvolutionBackwardFilter(cudnnHandle, &alpha, conv1Tensor,
                                                conv1, conv4Tensor, dfc1_3, conv4Desc,
                                                conv4bwfalgo, workspace, m_workspaceSize,
                                                &beta, conv4filterDesc, gconv4));
        
        checkCUDNN(cudnnConvolutionBackwardData(cudnnHandle, &alpha, conv4filterDesc,
    ...
    {    
        ...
        // Conv4
        checkCudaErrors(cublasSaxpy(cublasHandle, static_cast<int>(conv4.pconv.size()),
                                    &alpha, gconv4, 1, pconv4, 1));
        checkCudaErrors(cublasSaxpy(cublasHandle, static_cast<int>(conv4.pbias.size()),
                                    &alpha, gconv4bias, 1, pconv4bias, 1));
...
int main(int argc, char **argv)
{
    ...
    */
    ...
    if (!bRet || !FLAGS_pretrained)
    {
        ...
        float wconv4 = sqrt(3.0f / (conv4.kernel_size * conv4.kernel_size * conv4.in_channels));
        std::uniform_real_distribution<> dconv4(-wconv4, wconv4);        
        ...
        for (auto&& iter : conv3.pbias)
            iter = static_cast<float>(dconv3(gen));
        for (auto&& iter : conv4.pconv)
            iter = static_cast<float>(dconv4(gen));
        for (auto&& iter : conv4.pbias)
            iter = static_cast<float>(dconv4(gen));
    ...
    int channel_sum = conv1.out_channels + conv2.out_channels + conv3.out_channels + conv4.out_channels;
    // Forward propagation data
    float *d_data, *d_labels, *d_conv1, *d_conv2, *d_conv3, *d_conv4, *d_pool1, *d_concat, *d_fc1, *d_fc1relu, *d_fc2, *d_fc2smax;
    ...
    checkCudaErrors(cudaMalloc(&d_conv4,   sizeof(float) * context.m_batchSize * conv4.out_channels * conv4.out_height                  * conv4.out_width));
    checkCudaErrors(cudaMalloc(&d_pool1,   sizeof(float) * context.m_batchSize * conv1.out_channels * conv1.out_height                  * conv1.out_width));
    checkCudaErrors(cudaMalloc(&d_concat,  sizeof(float) * context.m_batchSize * channel_sum        * conv1.out_height                  * conv1.out_width));
    ...
    // Network parameters
    float *d_pconv1, *d_pconv1bias, *d_pconv2, *d_pconv2bias, *d_pconv3, *d_pconv3bias, *d_pconv4, *d_pconv4bias;
    ...
    checkCudaErrors(cudaMalloc(&d_pconv4,     sizeof(float) * conv4.pconv.size()));
    checkCudaErrors(cudaMalloc(&d_pconv4bias, sizeof(float) * conv4.pbias.size()));
    ...
    float *d_gconv1, *d_gconv1bias, *d_gconv2, *d_gconv2bias, *d_gconv3, *d_gconv3bias, *d_gconv4, *d_gconv4bias;
    ...
    checkCudaErrors(cudaMalloc(&d_gconv4,     sizeof(float) * conv4.pconv.size()));
    checkCudaErrors(cudaMalloc(&d_gconv4bias, sizeof(float) * conv4.pbias.size()));
    ...
    checkCudaErrors(cudaMalloc(&d_dconv4,   sizeof(float) * context.m_batchSize * conv1.out_channels * conv1.out_height * conv1.out_width));
    checkCudaErrors(cudaMalloc(&d_dfc11,    sizeof(float) * context.m_batchSize * conv2.out_channels * conv2.out_height * conv2.out_width));
    checkCudaErrors(cudaMalloc(&d_dfc12,    sizeof(float) * context.m_batchSize * conv3.out_channels * conv3.out_height * conv3.out_width));
    checkCudaErrors(cudaMalloc(&d_dfc13,    sizeof(float) * context.m_batchSize * conv4.out_channels * conv4.out_height * conv4.out_width));
    checkCudaErrors(cudaMalloc(&d_dfc14,    sizeof(float) * context.m_batchSize * conv1.out_channels * conv1.out_height * conv1.out_width)); // gradient for pool1
    ...
    checkCudaErrors(cudaMemcpyAsync(d_pconv4, &conv4.pconv[0],     sizeof(float) * conv4.pconv.size(),  cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpyAsync(d_pconv4bias, &conv4.pbias[0], sizeof(float) * conv4.pbias.size(),  cudaMemcpyHostToDevice));
    ...
    for (int iter = 0; iter < FLAGS_iterations; ++iter)
    {
        ...
        context.Backpropagation(conv1, conv2, conv3, conv4, pool1,
                                d_data, d_labels, d_conv1, d_conv2, d_conv3, d_conv4, d_pool1, d_concat, d_fc1, d_fc1relu, d_fc2, d_fc2smax, d_dlossdata,
    ...
    if (FLAGS_save_data)
    {
        ...
        checkCudaErrors(cudaMemcpy(&conv4.pconv[0], d_pconv4, sizeof(float) * conv4.pconv.size(), cudaMemcpyDeviceToHost));
        checkCudaErrors(cudaMemcpy(&conv4.pbias[0], d_pconv4bias, sizeof(float) * conv4.pbias.size(), cudaMemcpyDeviceToHost));
    ...
    if (classifications > 0)
    {
        ...
        TrainingContext test_context(FLAGS_gpu, 1, conv1, conv2, conv3, conv4, pool1, fc1, fc2);
    ...
    checkCudaErrors(cudaFree(d_conv4));
    checkCudaErrors(cudaFree(d_pool1));
    checkCudaErrors(cudaFree(d_concat));
    ...
    checkCudaErrors(cudaFree(d_pconv4));
    checkCudaErrors(cudaFree(d_pconv4bias));
    ...
    checkCudaErrors(cudaFree(d_gconv4));
    checkCudaErrors(cudaFree(d_gconv4bias));
    ...
    checkCudaErrors(cudaFree(d_dconv4));
    checkCudaErrors(cudaFree(d_dfc1));
    checkCudaErrors(cudaFree(d_dfc11));
    checkCudaErrors(cudaFree(d_dfc12));
    checkCudaErrors(cudaFree(d_dfc13));
    checkCudaErrors(cudaFree(d_dfc14));        
