																	...
																	Layer& layer6, Layer& layer7, Layer& input_layer)
{
	...
	if (pathNum == 0){
		fp_conv<<<GRID_SIZE, BLOCK_SIZE>>>(layer1.preact, input_layer.output, layer1.weight, layer1.kernel_size, 
			layer1.in_size, layer1.out_size, layer1.in_channel, layer1.out_channel, true);
		fp_bias_conv<<<GRID_SIZE, BLOCK_SIZE>>>(layer1.preact, layer1.bias, layer1.out_size, layer1.out_channel);
		apply_step_function<<<GRID_SIZE, BLOCK_SIZE>>>(layer1.preact, layer1.output, layer1.out_size * layer1.out_size * layer1.out_channel);
	}
	else if(pathNum == 1){
		fp_conv<<<GRID_SIZE, BLOCK_SIZE>>>(layer2.preact, input_layer.output, layer2.weight, layer2.kernel_size, 
			layer2.in_size, layer2.out_size, layer2.in_channel, layer2.out_channel, true);
		fp_bias_conv<<<GRID_SIZE, BLOCK_SIZE>>>(layer2.preact, layer2.bias, layer2.out_size, layer2.out_channel);
		apply_step_function<<<GRID_SIZE, BLOCK_SIZE>>>(layer2.preact, layer2.output, layer2.out_size * layer2.out_size * layer2.out_channel);
		
		fp_conv<<<GRID_SIZE, BLOCK_SIZE>>>(layer5.preact, layer2.output, layer5.weight, layer5.kernel_size, 
			layer5.in_size, layer5.out_size, layer5.in_channel, layer5.out_channel, true);
		fp_bias_conv<<<GRID_SIZE, BLOCK_SIZE>>>(layer5.preact, layer5.bias, layer5.out_size, layer5.out_channel);
		apply_step_function<<<GRID_SIZE, BLOCK_SIZE>>>(layer5.preact, layer5.output, layer5.out_size * layer5.out_size * layer5.out_channel);
	}
	else if(pathNum == 2){
		fp_conv<<<GRID_SIZE, BLOCK_SIZE>>>(layer3.preact, input_layer.output, layer3.weight, layer3.kernel_size, 
			layer3.in_size, layer3.out_size, layer3.in_channel, layer3.out_channel, true);
		fp_bias_conv<<<GRID_SIZE, BLOCK_SIZE>>>(layer3.preact, layer3.bias, layer3.out_size, layer3.out_channel);
		apply_step_function<<<GRID_SIZE, BLOCK_SIZE>>>(layer3.preact, layer3.output, layer3.out_size * layer3.out_size * layer3.out_channel);

		fp_conv<<<GRID_SIZE, BLOCK_SIZE>>>(layer6.preact, layer3.output, layer6.weight, layer6.kernel_size, 
			layer6.in_size, layer6.out_size, layer6.in_channel, layer6.out_channel, true);
		fp_bias_conv<<<GRID_SIZE, BLOCK_SIZE>>>(layer6.preact, layer6.bias, layer6.out_size, layer6.out_channel);
		apply_step_function<<<GRID_SIZE, BLOCK_SIZE>>>(layer6.preact, layer6.output, layer6.out_size * layer6.out_size * layer6.out_channel);
	}
	else{
		fp_maxpool<<<GRID_SIZE, BLOCK_SIZE>>>(layer4.output, input_layer.output, layer4.kernel_size, layer4.in_size, layer4.out_size, layer4.out_channel, true);

		fp_conv<<<GRID_SIZE, BLOCK_SIZE>>>(layer7.preact, layer4.output, layer7.weight, layer7.kernel_size, 
			layer7.in_size, layer7.out_size, layer7.in_channel, layer7.out_channel, true);
		fp_bias_conv<<<GRID_SIZE, BLOCK_SIZE>>>(layer7.preact, layer7.bias, layer7.out_size, layer7.out_channel);
		apply_step_function<<<GRID_SIZE, BLOCK_SIZE>>>(layer7.preact, layer7.output, layer7.out_size * layer7.out_size * layer7.out_channel);
	}

	cudaDeviceSynchronize();

	concat<<<GRID_SIZE,BLOCK_SIZE>>>(output_matrix, layer1.output, layer5.output, layer6.output, layer7.output,
...

{
	...
	//decat<<<GRID_SIZE,BLOCK_SIZE>>>(input_matrix, layer1.d_preact, layer2.d_preact, layer3.d_preact, layer4.d_preact,
	// 									input_layer.out_size, layer1.out_channel, layer2.out_channel, layer3.out_channel);
	if (pathNum == 0){
		bp_output_conv<<<GRID_SIZE, BLOCK_SIZE>>>(layer1.d_output, layer1.weight, slice1, layer1.in_size, layer1.kernel_size, 
																layer1.out_size, layer1.in_channel, layer1.out_channel, true, true);
		bp_preact_conv<<<GRID_SIZE, BLOCK_SIZE>>>(layer1.d_preact, layer1.d_output, layer1.preact, layer1.out_size, layer1.out_channel);
		bp_weight_conv<<<GRID_SIZE, BLOCK_SIZE>>>(layer1.d_weight, layer1.d_preact, layer1.output, layer1.kernel_size, layer1.in_size,
																layer1.out_size, layer1.in_channel, layer1.out_channel, false);
		bp_bias_conv<<<GRID_SIZE, BLOCK_SIZE>>>(layer1.bias, layer1.d_preact, layer1.out_size, layer1.out_channel);
	}
	else if(pathNum == 1){
		bp_output_conv<<<GRID_SIZE, BLOCK_SIZE>>>(layer5.d_output, layer5.weight, slice2, layer5.in_size, layer5.kernel_size, 
																layer5.out_size, layer5.in_channel, layer5.out_channel, true, true);
		bp_preact_conv<<<GRID_SIZE, BLOCK_SIZE>>>(layer5.d_preact, layer5.d_output, layer5.preact, layer5.out_size, layer5.out_channel);
		bp_weight_conv<<<GRID_SIZE, BLOCK_SIZE>>>(layer5.d_weight, layer5.d_preact, layer5.output, layer5.kernel_size, layer5.in_size,
																layer5.out_size, layer5.in_channel, layer5.out_channel, false);
		bp_bias_conv<<<GRID_SIZE, BLOCK_SIZE>>>(layer5.bias, layer5.d_preact, layer5.out_size, layer5.out_channel);

		bp_output_conv<<<GRID_SIZE, BLOCK_SIZE>>>(layer2.d_output, layer2.weight, layer5.d_preact, layer2.in_size, layer2.kernel_size, 
																layer2.out_size, layer2.in_channel, layer2.out_channel, true, true);
		bp_preact_conv<<<GRID_SIZE, BLOCK_SIZE>>>(layer2.d_preact, layer2.d_output, layer2.preact, layer2.out_size, layer2.out_channel);
		bp_weight_conv<<<GRID_SIZE, BLOCK_SIZE>>>(layer2.d_weight, layer2.d_preact, layer2.output, layer2.kernel_size, layer2.in_size,
																layer2.out_size, layer2.in_channel, layer2.out_channel, false);
		bp_bias_conv<<<GRID_SIZE, BLOCK_SIZE>>>(layer2.bias, layer2.d_preact, layer2.out_size, layer2.out_channel);
	}
	else if(pathNum == 2){
		bp_output_conv<<<GRID_SIZE, BLOCK_SIZE>>>(layer6.d_output, layer6.weight, slice3, layer6.in_size, layer6.kernel_size, 
																layer6.out_size, layer6.in_channel, layer6.out_channel, true, true);
		bp_preact_conv<<<GRID_SIZE, BLOCK_SIZE>>>(layer6.d_preact, layer6.d_output, layer6.preact, layer6.out_size, layer6.out_channel);
		bp_weight_conv<<<GRID_SIZE, BLOCK_SIZE>>>(layer6.d_weight, layer6.d_preact, layer6.output, layer6.kernel_size, layer6.in_size,
																layer6.out_size, layer6.in_channel, layer6.out_channel, false);
		bp_bias_conv<<<GRID_SIZE, BLOCK_SIZE>>>(layer6.bias, layer6.d_preact, layer6.out_size, layer6.out_channel);

		bp_output_conv<<<GRID_SIZE, BLOCK_SIZE>>>(layer3.d_output, layer3.weight, layer6.d_preact, layer3.in_size, layer3.kernel_size, 
																layer3.out_size, layer3.in_channel, layer3.out_channel, true, true);
		bp_preact_conv<<<GRID_SIZE, BLOCK_SIZE>>>(layer3.d_preact, layer3.d_output, layer3.preact, layer3.out_size, layer3.out_channel);
		bp_weight_conv<<<GRID_SIZE, BLOCK_SIZE>>>(layer3.d_weight, layer3.d_preact, layer3.output, layer3.kernel_size, layer3.in_size,
																layer3.out_size, layer3.in_channel, layer3.out_channel, false);
		bp_bias_conv<<<GRID_SIZE, BLOCK_SIZE>>>(layer3.bias, layer3.d_preact, layer3.out_size, layer3.out_channel);
	}
	else{
		bp_output_conv<<<GRID_SIZE, BLOCK_SIZE>>>(layer7.d_output, layer7.weight, slice4, layer7.in_size, layer7.kernel_size, 
																layer7.out_size, layer7.in_channel, layer7.out_channel, true, true);
		bp_preact_conv<<<GRID_SIZE, BLOCK_SIZE>>>(layer7.d_preact, layer7.d_output, layer7.preact, layer7.out_size, layer7.out_channel);
		bp_weight_conv<<<GRID_SIZE, BLOCK_SIZE>>>(layer7.d_weight, layer7.d_preact, layer7.output, layer7.kernel_size, layer7.in_size,
																layer7.out_size, layer7.in_channel, layer7.out_channel, false);
		bp_bias_conv<<<GRID_SIZE, BLOCK_SIZE>>>(layer7.bias, layer7.d_preact, layer7.out_size, layer7.out_channel);

		bp_maxpool<<<GRID_SIZE, BLOCK_SIZE>>>(layer4.d_preact, layer4.output, pre_maxpl, layer7.d_output, layer4.kernel_size,
														 layer4.in_size, layer4.out_size, layer4.out_channel, true);
	}

	cudaDeviceSynchronize();
	const int numElem = layer1.in_size * layer1.in_size * layer1.in_channel;
	sumGrad<<<GRID_SIZE,BLOCK_SIZE>>>(output_matrix, layer1.d_preact, layer2.d_preact, layer3.d_preact, layer4.d_preact, numElem);
}